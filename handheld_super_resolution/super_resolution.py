...
from .utils_dng import load_raw_burst
...
def process(burst_path, options=None, custom_params=None):
    ...
    ref_raw, raw_comp, ISO, tags, CFA, xyz2cam, ref_path = load_raw_burst(burst_path)


    if params.get("alpha", None) is not None:
    # 如果用户提供了自定义值，则优先使用它们。这部分逻辑保持不变。
        print("使用用户提供的 alpha 和 beta 噪声参数")
        alpha = params["alpha"]
        beta = params["beta"]

    elif tags.get('noise_profile') is not None:
    
        print("使用从 RAW 元数据中读取的通用噪声配置文件")
    

        alpha, beta = tags['noise_profile']

    else:
        
        warnings.warn("无法在 RAW 元数据中找到通用的噪声配置文件。将使用固定的默认值。")
        alpha = 0.0001
        beta = 0.0001
    ...
    # First, let's check if the input is a format that rawpy can't handle (like our .raw)
    is_custom_raw = ref_path.lower().endswith('.raw')

    if post_processing_enabled and not is_custom_raw:
        # --- Original Post-Processing Logic ---
        # This block will now ONLY run if post-processing is enabled AND the input is NOT a .raw file.
        if verbose_2:
            print('-- Post processing image')
        
        # This line is the one that was causing the error
        ...
        output_image = raw2rgb.postprocess(raw, handheld_output.copy_to_host(),
                                           ...
                                           xyz2cam, # This came from our loader
                                           params_pp['sharpening']
                                           ) 
    elif post_processing_enabled and is_custom_raw:
        # --- Simplified Post-Processing for .raw files ---
        # If the input is a .raw file, we can't do the full rawpy-based post-processing.
        # However, we can still apply some basic adjustments like sharpening if needed.
        # For now, we will just skip the complex steps and warn the user.
        warnings.warn("Skipping rawpy-based post-processing (color correction, etc.) because the input is a custom .raw file.")
        
        # The output from the `main` function is a linear RGB image.
        # We can apply a simple gamma correction to make it viewable.
        output_image = handheld_output.copy_to_host()
        if params_pp['do gamma']:
            # Apply a standard sRGB gamma correction
            output_image = np.power(output_image, 1/2.2)

    else: # post_processing_enabled is False
        output_image = handheld_output.copy_to_host()
